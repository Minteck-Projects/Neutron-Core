...
if (isset($_COOKIE['ADMIN_TOKEN'])) {
    if (file_exists($_SERVER['DOCUMENT_ROOT'] . "/data/tokens/" . $_COOKIE['ADMIN_TOKEN'])) {
        ...
        if (isset($_GET['id'])) {
            die("<script>location.href = '/cms-special/admin/?pr=/cms-special/admin/store/package/&pa=?id=" . $_GET['id'] . "'</script>");
        } else {
            die("<script>location.href = '/cms-special/admin/?pr=/cms-special/admin/store/package&pa='</script>");
        }
    }
} else {
    if (isset($_GET['id'])) {
...
<head>
    ...
    if ($ready) {
        echo("{$name} - CMS Store - Administration du site - " . file_get_contents($_SERVER['DOCUMENT_ROOT'] . "/data/webcontent/sitename"));
    } else {
